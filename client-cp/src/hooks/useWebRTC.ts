...
import type {ConnectionState, ICEMessage, SDPMessage, WebRTCMessage} from "../types/webrtc";
...
const useWebRTC = (url: string, roomId: string): WebRTCHook => {
	...
	const initializeMediaStream = useCallback(async () => {
		try {
			...
			updateConnectionState("MediaStream Error",
				error instanceof Error ? error : new Error("Failed to get media stream")
			);
			return null;
		}
	...
	const setupPeerConnection = useCallback(async (stream?: MediaStream) => {
		try {
			...
			peerConnection.current.oniceconnectionstatechange = () => {
				updateConnectionState(`ICE: ${peerConnection.current?.iceConnectionState}`);
			};

			peerConnection.current.onconnectionstatechange = () => {
				updateConnectionState(`Connection: ${peerConnection.current?.connectionState}`);
			};
			...
			updateConnectionState("Error", error instanceof Error ? error : new Error("Setup failed"));
		}
	...
	const handleSDP = useCallback(async (sdp: RTCSessionDescriptionInit) => {
		...
		try {
			...
			updateConnectionState("SDP Error", error instanceof Error ? error : new Error("SDP handling failed"));
		}
	}, [roomId, updateConnectionState]);
	...
	const handleICECandidate = useCallback(async (candidate: RTCIceCandidateInit) => {
		...
		try {
			...
			updateConnectionState("ICE Error", error instanceof Error ? error : new Error("ICE handling failed"));
		}
	}, [updateConnectionState]);
	...
	}, [roomId, handleMessage, setupPeerConnection, updateConnectionState, url, cleanupWebSocket]);
